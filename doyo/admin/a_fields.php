...
class a_fields extends syController
{
	...
	function index(){
		...
		$this->lists=$this->ClassF->syPager($this->gopage,15)->findAll($w);
	...
	function info(){
		...
		foreach($v as $f){
			switch ($f['fieldstype']){
				case 'varchar':
					if($f['fieldslong']>255){
						$t='<dd><textarea name="'.$f['fields'].'" style="width:300px; height:50px;" class="int">'.$c[$f['fields']].'</textarea></dd>';
					}else{
						$t='<dd><input name="'.$f['fields'].'" type="text" class="int" value="'.$c[$f['fields']].'" /></dd><dd class="t">最多'.$f['fieldslong'].'个字</dd>';
					}
				break;
				case 'text':
					$t='<script type="text/javascript">$("#'.$f['fields'].'").xheditor()</script>';
					$t.='<dd><textarea name="'.$f['fields'].'" id="'.$f['fields'].'" class="';
                    $t.="xheditor  {html5Upload:false,upLinkUrl:'!doyo.php?c=uploads&a=loadup&fileover=2&isfiles=filedata',upImgUrl:'!doyo.php?c=uploads&a=loadup&fileover=2&multi=1&isfiles=filedata&fileExt=*.jpg;*.jpeg;*.gif;*.png',upFlashUrl:'!doyo.php?c=uploads&a=loadup&fileover=2&isfiles=filedata',upMediaUrl:'!doyo.php?c=uploads&a=loadup&fileover=2&isfiles=filedata'}";
                    $t.='" style="width:auto;height:150px;">'.$c[$f['fields']].'</textarea></dd>';
				break;
				case 'int':
				...
				break;
				case 'files':
				$t='<dd><input name="'.$f['fields'].'" id="'.$f['fields'].'" type="text" class="int" value="'.$c[$f['fields']].'" /></dd><dd><iframe frameborder="0" width="300" height="26" scrolling="No" id="flitpic" name="flitpic" src="doyo.php?c=uploads&a=loadup&inputid='.$f['fields'].'"></iframe></dd>';
				break;
				case 'fileall':
				...
				if($c[$f['fields']]){
					...
					foreach(explode('|-|',$c[$f['fields']]) as $v){
						...
						$t.='<input name="'.$f['fields'].'file[]" type="hidden" value="'.$s[0].'" /><br /><input name="'.$f['fields'].'txt[]" type="text" value="'.$s[1].'" class="int" style="width:52px;height:12px;" /><br /><a onclick=delfieldall("f_'.$fnames.'") style="width:43;padding-left:17px;cursor:pointer;">删除</a></dd>';
						$n++;
					}
				}
				$t.='</dl>';
				$t.='<dl><dt>'.$f['fieldsname'].'：</dt><dd><iframe frameborder="0" width="300" height="26" scrolling="No" id="flitpic" name="flitpic" src="doyo.php?c=uploads&a=loadup&inputid='.$f['fields'].'&multi=1&fileover=1"></iframe></dd></dl>';
				break;
				case 'select':
