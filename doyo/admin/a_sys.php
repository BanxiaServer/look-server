...
class a_sys extends syController
{
	...
	function index(){
		if($this->syArgs('run')==1){
			...
			@fclose($fp_tp);
			$config=array('http_path','site_title','site_keywords','site_description','view_themes','site_html','site_html_dir','site_html_rules','cache_auto','cache_time','filetype','filesize','imgwater','imgwater_t','imgcaling','img_w','img_h','comment_audit','comment_user','rewrite_open');
			foreach($config as $v){
				if (strpos(',site_html,cache_auto,cache_time,filesize,imgwater,imgwater_t,imgcaling,img_w,img_h,comment_audit,comment_user,',$v)){
			...
			@fclose($inc_tp);
			$inc=array('rewrite_open','rewrite_article','rewrite_article_type','rewrite_product','rewrite_product_type','rewrite_message_type','rewrite_special','rewrite_labelcus_custom','rewrite_dir');
			foreach($inc as $v){
				...
				if($v=='rewrite_product'){
					...
					message_a("product规则中{id}与{file}必须至少包含一个，并且必须包含product}");
					}
				}
	...
	private function rewrite_for($d,$r){
		$num=1;
		foreach($d[1] as $k=>$v){
			if(stripos(',,type,special,article,product,message,,',','.$v.',')){
				if(stripos(',,special,article,product,message,,',','.$v.',')){
					$u.='&c='.$v;
				}else{
					$u.='&a='.$v;
				}
