...
class a_label extends syController
{
	...
	function output(){
		if($this->syArgs('as',1)){
			if(preg_match("/^[a-zA-Z][a-zA-Z0-9]*$/",$this->syArgs('as',1))!=0){$as=$this->syArgs('as',1);$asv=' as="'.$this->syArgs('as',1).'"';}else{echo '调用标识必须为英文或数字，并且以英文开头';exit;}
		}else{$as='v';}
...
if($v['fieldstype']=='fileall'){
	...
	$l.='<tr><td><strong>'.$v['fieldsname'].':</strong> <span>{$'.$as.'[';$l.="'".$v['fields']."'";$l.=']}</span></td></tr>';
				...
				$l.='</table><span>{/loop}</span><br /><br /><br /><br />';
			break;
			case 'product':
				$w='table="product"';
				if($this->syArgs('tid'))$w.=' tid="'.$this->syArgs('tid').'"';
