...
class a_html extends syController
{
	...
	function clear(){
		ob_implicit_flush(1);
		ob_end_flush();
		echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" /><link href="doyo/admin/template/style/admin.css" rel="stylesheet" type="text/css" /><script src="include/js/jquery.js" type="text/javascript"></script><script type="text/javascript">function goclear(ctxt){$("#clear").html(ctxt);}</script><div class="main"><div class="progress" id="clear">正在统计需更新数量...</div></div>';
		deleteDir($GLOBALS['G_DY']['sp_cache']);
		deleteDir($GLOBALS['G_DY']['view']['config']['template_tpl']);
		$ww='';$t=$this->syArgs('t',1);
		if($t==1||$t==0){
			if($t==0){$m=array('article','product');}else{$m=array($this->syArgs('molds',1));}
			foreach($m as $mv){
				...
				$sql='select id,addtime,htmlfile,body from '.$GLOBALS["G_DY"]["db"]["prefix"].$mv.' a left join '.$GLOBALS["G_DY"]["db"]["prefix"].$mv.'_field b on (a.id=b.aid) where isshow=1'.$ww.' and mrank=0 and mgold=0';
	...
	private function chtml_molds($molds,$a,$anum,$isnum) {
		foreach($a as $v){
			...
			$c_html_f=html_rules($molds,$v['addtime'],$v['id'],$v['htmlfile']);
	...
	private function chtml_classtype($a) {
		...
		foreach($a as $v){
			...
			$alls=$cl->syPager(1,$v['listnum'])->findAll(' isshow=1 and tid in('.$this->types->leafid($v['tid']).') ',null,'tid,isshow');
	...
	private function chtml_special($a) {
		...
		foreach($a as $v){
			...
			$cl=syClass('c_'.$v['molds']);
			$alls=$cl->syPager(1,$v['listnum'])->findAll(array('sid'=>$v['sid'],'isshow'=>1),null,'sid,isshow');
			$pages=$cl->syPager()->getPager();
			if($pages['total_page']>1){
				for ($i = 2; $i <= $pages['total_page']; $i++) {
