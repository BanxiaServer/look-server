...
class a_classtypes extends syController
{
			...
			$newVerifier=$this->ClassT->syVerifier($this->newrow);
				if(false == $newVerifier){
					...
					if($this->newrow['t_index']=='' || $this->newrow['t_list']=='' || $this->newrow['t_listimg']=='' || $this->newrow['t_listb']=='' || $this->newrow['t_content']==''){
						$thistypes=syDB('classtype')->find(array('tid'=>$this->newrow['pid']),null,'molds,t_index,t_list,t_listimg,t_listb,t_content ');
						$tm=array();
	...
	function edit(){
		...
		if ($this->syArgs('run')==1){
			if($this->carray['tid']==$this->newrow['pid'])message_a("上级栏目与本栏目不能相同，请重新选择");
			$newVerifier=$this->ClassT->syVerifier($this->newrow);
			if(false == $newVerifier){
				deleteDir($GLOBALS['G_DY']['sp_cache']);
	...
	function del(){
		...
		if ($this->syArgs('run')==1){
			...
			foreach (explode(',',$tida) as $v){
				$types=$this->ClassT->find(array('tid'=>$v));
				$db=$this->db.$types['molds'];
				if(syDB('molds')->find(array('molds'=>$types['molds']))){
					syDB($types['molds'])->runSql('DELETE '.$db.','.$db.'_field FROM '.$db.','.$db.'_field WHERE '.$db.'.id='.$db.'_field.aid and '.$db.'.tid in('.$tida.')');
				}
			}
			deleteDir($GLOBALS['G_DY']['sp_cache']);
