...
class product extends syController
{
	...
	function index(){
		...
		if($this->product['mrank']>0){
			if(empty($_SESSION['member']))message('本内容需要登录才可访问');
			if(membergroup($this->product['mrank'],'weight')>$this->member_g['weight'])message('本内容需要['.membergroup($this->product['mrank'],'name').']及以上用户才可访问');
		}
		if($this->product['mgold']>0){
			//扣除金币操作
			message('本内容需要消费['.$this->product['mgold'].']金币');
		}
		...
		$prev=syDB('product')->find(' addtime<'.$this->product['addtime'].$prev_next_w,'addtime desc',$prev_next_f);
		if($prev){
			$prev['url']=html_url('product',$prev);
			$this->aprev=$prev;
		}
		$next=syDB('product')->find(' addtime>'.$this->product['addtime'].$prev_next_w,'addtime',$prev_next_f);
		if($next){
		...
		$this->type=syDB('classtype')->find(" molds='product' and tid=".$this->product['tid']." ",null,'tid,classname,litpic,keywords,description,t_content,htmldir,htmlfile,mrank,msubmit');
	...
	function type(){
		...
		if($this->type['isindex']==1||$this->type['isindex']==2){
			...
			$sql='select id,tid,title,style,trait,gourl,addtime,hits,litpic,orders,price,mrank,mgold,isshow,description,htmlurl,htmlfile,user';
			if($f){
