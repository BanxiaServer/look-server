	...
	public function make($spurl, $alias_url = null, $todir = 1)
	{
		...
		@list($geturl, $controller, $action, $args, $anchor) = $spurl;
			...
			if( FALSE === $cachedata ){
				...
				if( FALSE === $cachedata )syError("无法从网络获取页面数据，请检查：<br />1. Url生成地址是否正确！<a href='{$remoteurl}' target='_blank'>点击这里测试</a>。<br />2. 设置php.ini的allow_url_fopen为On。<br />3. 检查是否防火墙阻止了APACHE/PHP访问网络。<br />4. 建议安装CURL函数库。");
			}
